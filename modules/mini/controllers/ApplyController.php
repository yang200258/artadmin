...
class ApplyController extends Controller
{
    ...
    public function actionAdd()
    {
        ...
        try {
            ...
            $apply->bm_continuous = $continuous_level ? Pdf::createPdfApply($apply->id, $continuous_level) : '';// 连考报名表
            $apply->save(false);
            //计算考收取费用，如果连考收两级费用
    ...
    public function actionList()
    {
        ...
        $total = $model->count();
        $list = $model->with(['pay','exam'])->orderBy('id desc')->offset($this->offset)->limit($this->limit)->asArray()->all();
    ...
    {
        $haikou = User::find()->select('id, organ_address, name, phone')->where(['organ_area' => 0])->andWhere(['type' => 2])->asArray()->all();
        array_walk($haikou, function (&$val){
            $val['phone'] = explode(',', $val['phone']);
        });

        $qita = User::find()->select('id, organ_address, name, phone')->where(['organ_area' => 1])->andWhere(['type' => 2])->asArray()->all();
        array_walk($qita, function (&$val){
            $val['phone'] = explode(',', $val['phone']);
        });
